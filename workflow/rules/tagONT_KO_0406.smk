...
rule determine_best_read_ctg:
  input:
    bn="/project/logsdon_shared/projects/HGSVC3/GAVISUNK/workflow/scripts/read_pair/target/release/read_pair",
    locs = rules.bed_convert.output.locs,
    ONT = "temp/{sample}/reads/{hap}_{scatteritem}.fq.gz",
  output:
    pair="results/{sample}/sunkpos/{hap}_{scatteritem}.pair",
    sunk_pos="results/{sample}/sunkpos/{hap}_{scatteritem}.sunkpos",
    sunk_num="results/{sample}/sunkpos/{hap}_{scatteritem}.sunknum",
  params:
    kmer_len=config["SUNK_len"],
    sunk_dis=0.02,
    tmp_fasta=lambda wc, input: str(input.ONT).replace("fq.gz", "fa"),
    outdir=lambda wc, output: os.path.dirname(output.pair),
  log:
    "logs/{sample}/{hap}_{scatteritem}_determine_best_read_ctg.log"  
  benchmark:
    "benchmarks/{sample}/{hap}_{scatteritem}_determine_best_read_ctg.tsv"  
  threads: 10
  resources:
    mem="150GB",
  shell:
    """
    seqkit fq2fa {input.ONT} -o {params.tmp_fasta}
    {input.bn} \
    -i {params.tmp_fasta} \
    -s {input.locs} \
    -o {params.outdir} \
    -k {params.kmer_len} \
    -d {params.sunk_dis} &> {log}
    rm {params.tmp_fasta}
    """
...
rule bad_sunks:
  input:
    hap1_fai = lambda wildcards: manifest_df.at[wildcards.sample, "hap1_asm_fai"],
    hap2_fai = lambda wildcards: manifest_df.at[wildcards.sample, "hap2_asm_fai"],
...
checkpoint split_sunkpos:
  ...
  shell:
    """
    python workflow/scripts/split_locs_cli.py {input.ONT_pos} {input.kmer_loc} {params.outdir} {wildcards.hap} 2> {log}
    """
...
rule slop_gaps:
  input:
    ...
    fai =  lambda wildcards: manifest_df.at[wildcards.sample,f"{wildcards.hap}_asm_fai"]
  output:
