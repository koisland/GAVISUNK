...
def main():
    print("hap1")
    fai = pd.read_csv(
        args.fai1, sep="\t", header=None, names=["contig", "length", "cumlen", "3", "4"]
    )
    ...
    pd.DataFrame(nodata).to_csv(
        outdir + "hap1.nodata.bed", header=False, sep="\t", index=False
    )
    pd.DataFrame(outbreaks).to_csv(
        outdir + "hap1.gaps.bed", header=False, sep="\t", index=False
    )

    fai = pd.read_csv(
        args.fai2, sep="\t", header=None, names=["contig", "length", "cumlen", "3", "4"]
    )
    contigs = fai.contig.tolist()
    ...
    print("no data: ", len(nodata), bp2)
    pd.DataFrame(nodata).to_csv(
        outdir + "hap2.nodata.bed", header=False, sep="\t", index=False
    )
    pd.DataFrame(outbreaks).to_csv(
        outdir + "hap2.gaps.bed", header=False, sep="\t", index=False
    )
